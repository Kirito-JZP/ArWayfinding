...
public class NavigationUtils {
    ...
    private static List<com.google.maps.model.LatLng> filterValidWaypoints(RouteDto route,
        ...
        for (LocationDto waypoint : route.getWaypoints()) {
            if (isLocationAheadOfReference(route, waypoint, currentLocation)) {
                results.add(LatLngUtils.convert(LatLngUtils.getLatLngFromDto(waypoint)));
            }
        }
        return results;
    }
    ...
    public static Pair<List<RouteDto>, LatLngBounds> findRoute(
        ...
        if (startLocDto != null && targetLocDto != null) {
            LatLng orig = LatLngUtils.getLatLngFromDto(startLocDto);
            LatLng dest = LatLngUtils.getLatLngFromDto(targetLocDto);
            List<RouteDto> routes = new ArrayList<>();
            try {
                DirectionsApiRequest request =
                        new DirectionsApiRequest(ArWayfindingApp.getGeoApiContext())
                                .origin(LatLngUtils.convert(orig))
                                .destination(LatLngUtils.convert(dest))
//                                .alternatives(true)
                                .mode(mode);
                ...
                for (DirectionsRoute r : result.routes) {
                    ...
                    for (int i = 0; i < batchNum; i++) {
                        ...
                        for (int j = startIndex; j < endIndex; j++) {
                            ...
                            List<LatLng> lls = LatLngUtils.convertMap2GMS(sub);
                            List<SnappedPoint> snappedPoints =
                                    PlaceUtils.nearestRoads(lls);
    ...
    public static RouteDto.RouteStep generateRouteStepWithoutPlaceID(DirectionsStep step) {
        ...
        Context context = ArWayfindingApp.getContext();
        ...
        options.addAll(LatLngUtils.convertMap2GMS(step.polyline.decodePath()))
                .color(colour)
                .width(25)
    ...
    private static int findClosestStep(RouteDto route, LocationDto location) {
        ...
        for (int i = 0; i < steps.size(); i++) {
            ...
            for (int j = 0; j < points.size(); j++) {
                ...
                LatLng endLocation = LatLngUtils.getLatLngFromDto(location);
                float[] results = {0.0f};
    ...
    public static void updateRouteState(RouteDto route, LocationDto currentLocation) {
        ...
        if (index != -1) {
            ...
            double min_distance = LatLngUtils.calcDistance(points.get(0), LatLngUtils.getLatLngFromDto(currentLocation));
            for (int i = 0; i < points.size(); i++) {
                double distance = LatLngUtils.calcDistance(points.get(i), LatLngUtils.getLatLngFromDto(currentLocation));
    ...
    public static List<com.google.maps.model.LatLng> getLatLngFromWaypoints(@Nullable RouteDto currentRoute) {
        if (currentRoute == null || currentRoute.getWaypoints().isEmpty()) {
            ...
            return currentRoute.getWaypoints()
                    .stream()
                    .map(waypoint ->
                            LatLngUtils.convert(LatLngUtils.getLatLngFromDto(waypoint))
